...
However, if the driver is controlling the vehicle via AR and choose to receive two bonus Matrix simple actions, they can use those actions to control the vehicle, effectively doing so for "free." This does mean they cannot receive any bonus meatspace actions, however. See the [action economy page]({{<relref "action_economy">}}) for more information.
